...
Java_dev_governance_android_app_agent_LlamaCppNative_nativeInit(
    ...
    // Planner prompts are ~233 tokens; 512 gives headroom without
    // over-allocating the compute buffer.
    //
    // Thread count: use available cores (capped at 8 to avoid contention).
    // On real phones this is typically 4-8; on the x86_64 emulator this
    // matches the AVD's hw.cpu.ncore, letting the host CPU do real work.
    int hw_threads = std::thread::hardware_concurrency();
    int n_threads  = (hw_threads > 0) ? std::min(hw_threads, 8) : 4;
    LOGI("nativeInit: hw_threads=%d, using n_threads=%d", hw_threads, n_threads);
